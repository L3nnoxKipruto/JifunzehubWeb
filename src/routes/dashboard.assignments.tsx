...
function AssignmentsComponent() {
  const assignments = [
    {
      id: 1,
      title: "Packet Tracer Lab 2: Subnetting",
    ...
    {
      id: 3,
      title: "Solar Panel Angle Calculation",
      course: "Solar Power Systems",
      deadline: "2 days ago",
      type: "Mathematical Practical",
      lecturer: "Dr. Omondi",
      status: "Synced",
      priority: "low",
      ...
      <div className="space-y-8">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-background border border-border/60 rounded-2xl p-4 shadow-sm">
          <div className="flex items-center gap-3 flex-1 w-full md:w-auto">
            <div className="relative flex-1 max-w-sm">
              ...
              <Input
                ...
                className="pl-10 h-9 bg-muted/30 border-border/60"
              />
            </div>
          ...
          <div className="flex items-center gap-4 overflow-x-auto pb-1 md:pb-0 w-full md:w-auto">
            <div className="flex gap-2">
              <Badge
                variant="secondary"
                className="rounded-full px-3 py-1 bg-primary/10 text-primary border-none"
              >
                Total: 4
              </Badge>
              <Badge
                variant="secondary"
                className="rounded-full px-3 py-1 bg-amber-500/10 text-amber-600 border-none"
              >
                Due Soon: 2
              </Badge>
              <Badge
                variant="secondary"
                className="rounded-full px-3 py-1 bg-success/10 text-success border-none"
              >
                Graded: 2
              </Badge>
            </div>
        ...
        <div className="grid gap-4">
          {assignments.map((assignment) => (
            <Card
              key={assignment.id}
              className="border-border/60 hover:shadow-soft transition-all duration-300 group overflow-hidden bg-background"
            >
              <div
                ...
                <div className="flex flex-col md:flex-row items-stretch">
                  <div className="flex-1 p-5 space-y-4">
                      ...
                      <div className="space-y-1">
                        <div className="flex items-center gap-2">
                          ...
                          {assignment.priority === "high" && (
                            <Badge className="bg-destructive/10 text-destructive border-none text-[10px] uppercase font-bold px-1.5 h-4">
                              Priority
                            </Badge>
                          )}
                        </div>
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end" className="w-48">
                            <DropdownMenuItem>
                              <ExternalLink className="w-4 h-4 mr-2" /> View Rubric
                            </DropdownMenuItem>
                            <DropdownMenuItem>
                              <MessageSquare className="w-4 h-4 mr-2" /> Contact Lecturer
                            </DropdownMenuItem>
                            <DropdownMenuSeparator />
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 pt-2">
                      <div className="space-y-1">
                        ...
                        <p
                          className={`text-sm font-semibold ${assignment.status === "Pending" ? "text-destructive" : ""}`}
                        >
                          {assignment.deadline}
                        </p>
                      </div>
                      <div className="space-y-1">
                        <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest flex items-center gap-1">
                          <FileType className="w-3 h-3" /> Type
                        </p>
                        <p className="text-sm font-semibold">{assignment.submissionType}</p>
                      </div>
                      <div className="space-y-1">
                        <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest flex items-center gap-1">
                          <Users className="w-3 h-3" /> Lecturer
                        </p>
                  ...
                  <div className="bg-muted/20 border-t md:border-t-0 md:border-l border-border/50 p-5 flex md:flex-col justify-center gap-3 md:w-56 shrink-0">
                    {assignment.status === "Pending" ? (
                      ...
                      <Button
                        variant="outline"
                        className="w-full bg-background text-amber-600 border-amber-200 hover:bg-amber-50"
                      >
        ...
        <div className="bg-primary/5 border border-primary/20 rounded-2xl p-8 text-center space-y-4">
          ...
          <div className="max-w-md mx-auto space-y-2">
            ...
            <p className="text-sm text-muted-foreground">
              You can upload your practical assignments even when offline. They will be queued for
              synchronization with the campus server automatically.
            </p>
          </div>
          <div className="flex justify-center gap-3 pt-2">
            <Badge variant="outline" className="bg-background">
              Supports: PDF, DOCX, ZIP, PKT
            </Badge>
            <Badge variant="outline" className="bg-background">
              Max Size: 50MB
            </Badge>
