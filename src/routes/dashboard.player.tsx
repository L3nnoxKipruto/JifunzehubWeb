...
import {
  CheckCircle2, Download, FileText, Lock, Maximize2, MessageSquare,
  Pause, Play, Settings as SettingsIcon, SkipForward, Volume2, WifiOff,
} from "lucide-react";
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_360px]">
        <div>
          <div className="mb-3 flex flex-wrap items-center gap-2">
            ...
            <Badge variant="outline" className="border-success/40 text-success"><WifiOff className="mr-1 h-3 w-3" /> Available offline</Badge>
          ...
          <p className="mt-1 text-sm text-muted-foreground">Networking Essentials • Module 3 • Lesson 14</p>
          ...
          <Tabs defaultValue="notes" className="mt-6">
            <TabsList>
              <TabsTrigger value="notes"><FileText className="mr-1 h-4 w-4" /> Notes</TabsTrigger>
              <TabsTrigger value="resources">Resources</TabsTrigger>
              <TabsTrigger value="assessment">Assessment</TabsTrigger>
              <TabsTrigger value="discuss"><MessageSquare className="mr-1 h-4 w-4" /> Discussion</TabsTrigger>
            </TabsList>
            <TabsContent value="notes">
              <Textarea placeholder="Write your notes here. They'll sync to your account when online…" rows={6} />
              <p className="mt-2 text-xs text-muted-foreground">Auto-saved locally • Last save 12s ago</p>
            </TabsContent>
            <TabsContent value="resources" className="space-y-2">
              {[
                ...
                <div key={n} className="flex items-center justify-between rounded-lg border border-border/60 p-3 text-sm">
                  <div className="flex items-center gap-2"><FileText className="h-4 w-4 text-accent" /> {n}</div>
                  <Button variant="ghost" size="sm"><Download className="mr-1 h-4 w-4" /> {s}</Button>
                </div>
              ))}
            </TabsContent>
            <TabsContent value="assessment">
              <Card className="border-border/60"><CardContent className="p-5 text-sm">
                <p className="font-medium">Quiz: Subnetting in practice</p>
                <p className="mt-1 text-muted-foreground">10 questions • 15 min • Submission queues offline</p>
                <Button className="mt-4 bg-gradient-primary text-primary-foreground hover:opacity-95">Start quiz</Button>
              </CardContent></Card>
            </TabsContent>
            <TabsContent value="discuss">
              <p className="text-sm text-muted-foreground">No messages yet. Discussion syncs once you reconnect.</p>
            </TabsContent>
          </Tabs>
        </div>
            ...
            <div className="max-h-[640px] overflow-auto">
              {sections.map((s) => (
                <div key={s.title} className="border-b border-border/60 p-3">
                  <p className="px-2 py-1 text-xs font-medium uppercase tracking-wider text-muted-foreground">{s.title}</p>
                  {s.items.map((it) => (
                    <button
                      key={it.t}
                    ...
                    >
                      <span className="mt-0.5">
                        {(it as any).done ? <CheckCircle2 className="h-4 w-4 text-success" /> :
                         (it as any).locked ? <Lock className="h-4 w-4 text-muted-foreground" /> :
                         <Play className="h-4 w-4 text-accent" />}
                      </span>
                      <span className="flex-1">
                        <span className={`block ${(it as any).current ? "font-semibold" : ""}`}>{it.t}</span>
