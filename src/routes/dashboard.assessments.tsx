...
function AssessmentsComponent() {
  const upcomingExams = [
    {
      id: 1,
      title: "Mid-Term: Networking & IP Subnetting",
  ...
  return (
    <DashboardLayout
      role="student"
      title="Assessments"
      subtitle="Take exams, practice your skills with quizzes, and review your performance."
    >
      <div className="space-y-8">
        ...
        <Tabs defaultValue="upcoming" className="w-full">
          <TabsList className="bg-muted/50 border border-border/50 h-11 w-full sm:w-auto p-1">
            <TabsTrigger value="upcoming" className="flex-1 sm:flex-none gap-2 px-6">
              <Timer className="w-4 h-4" /> Upcoming Exams
            </TabsTrigger>
            <TabsTrigger value="practice" className="flex-1 sm:flex-none gap-2 px-6">
              <BookOpen className="w-4 h-4" /> Practice Quizzes
            </TabsTrigger>
            <TabsTrigger value="completed" className="flex-1 sm:flex-none gap-2 px-6">
          ...
          <TabsContent value="upcoming" className="mt-6">
            <div className="grid gap-6 md:grid-cols-2">
              {upcomingExams.map((exam) => (
                <Card
                  key={exam.id}
                  className="border-border/60 hover:shadow-elegant transition-all duration-300 bg-background overflow-hidden flex flex-col"
                >
                  <div
                    className={`h-1.5 w-full ${exam.status === "Ready" ? "bg-success" : "bg-muted"}`}
                  ></div>
                  <CardHeader className="p-5 pb-2">
                    <div className="flex justify-between items-start mb-2">
                      <Badge
                        variant="outline"
                        className="text-[10px] font-bold tracking-tight uppercase bg-muted/30"
                      >
                        {exam.course}
                      </Badge>
                      <Badge
                    ...
                    <div className="flex items-center gap-4 mt-2 text-xs text-muted-foreground">
                      ...
                      <span className="flex items-center gap-1">
                        <HelpCircle className="w-3 h-3" /> {exam.questions} Questions
                      </span>
                      <span className="flex items-center gap-1">
                        <Target className="w-3 h-3" /> Pass: {exam.passMark}
                      </span>
                    </div>
                  </CardHeader>
                  <CardContent className="p-5 pt-4 space-y-4 flex-1">
                    <div className="flex items-center gap-2 p-3 rounded-lg bg-muted/40 border border-border/50">
                      <RefreshCw
                        className={`w-4 h-4 ${exam.offlineCompatible ? "text-success" : "text-muted-foreground"}`}
                      />
                      <p className="text-xs font-medium">
                        {exam.offlineCompatible
                          ? "Full Offline Exam Mode Compatible"
                          : "Online Only (Requires LAN)"}
                      </p>
                    </div>
                    {exam.status === "Locked" && (
                  ...
                  <CardContent className="p-5 flex items-center justify-between">
                    <div className="space-y-1">
                      ...
                      <p className="text-xs text-muted-foreground">
                        {quiz.attempts} Previous Attempts • Best: {quiz.bestScore}
                      </p>
                    </div>
            ...
            <Card className="border-border/60 overflow-hidden">
              <div className="p-8 text-center space-y-3">
                ...
                <p className="text-sm text-muted-foreground max-w-xs mx-auto">
                  Complete your first assessment to see your scores and performance breakdown here.
                </p>
              </div>
            </Card>
          </TabsContent>
