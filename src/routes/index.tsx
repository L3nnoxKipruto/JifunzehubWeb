...
import {
  WifiOff, Usb, Radio, Smartphone, Gauge, Globe2, Search, ArrowRight,
  CheckCircle2, GraduationCap, Building2, Users, BookOpen, Sparkles,
  Wrench, Plug, Hammer, Car, UtensilsCrossed, Scissors, HardHat,
} from "lucide-react";
...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Offline-first TVET learning platform for institutions in low-connectivity regions. Sync over USB, local hotspot, or the cloud." },
      { property: "og:title", content: "JifunzeHub — Offline-first TVET Learning" },
      { property: "og:description", content: "Inclusive transformation and empowerment through offline-capable digital education for TVET colleges." },
    ],
  }),
  component: Landing,
});
    ...
    <section className="relative overflow-hidden bg-gradient-hero text-primary-foreground">
      ...
      <div className="container relative mx-auto grid gap-10 px-4 py-20 lg:grid-cols-2 lg:items-center lg:py-28">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
          <Badge variant="secondary" className="mb-5 gap-1.5 bg-white/15 text-primary-foreground hover:bg-white/20">
          ...
          <p className="mt-5 max-w-xl text-base text-primary-foreground/85 md:text-lg">
            JifunzeHub is the offline-synchronized e-learning platform built for TVET colleges in low-bandwidth and rural environments. Learn, teach, and assess — internet optional.
          </p>
          ...
          <div className="mt-7 flex max-w-xl items-center gap-2 rounded-xl border border-white/20 bg-white/10 p-2 backdrop-blur">
            ...
            />
            <Button className="bg-accent text-accent-foreground hover:bg-accent-glow">Search</Button>
          </div>

          <div className="mt-6 flex flex-wrap gap-3">
            <Button asChild size="lg" className="bg-white text-primary hover:bg-white/90">
              <Link to="/register">Start learning <ArrowRight className="ml-1 h-4 w-4" /></Link>
            </Button>
          ...
          <div className="mt-8 flex flex-wrap gap-x-6 gap-y-2 text-sm text-primary-foreground/80">
            {["No internet required", "Works on low-end devices", "Kiswahili & English"].map((t) => (
...
function TrustBar() {
  const logos = ["Nairobi TVET", "Kisumu Polytechnic", "Arusha Tech", "Kigali Skills", "Mombasa Vocational", "Dodoma TI"];
  return (
...
const features = [
  { icon: WifiOff, title: "Offline-first learning", desc: "Lessons, videos and quizzes work without an internet connection. Progress is saved locally." },
  { icon: Usb, title: "USB synchronization", desc: "Move entire course packages between campuses and devices using a simple USB drive." },
  { icon: Radio, title: "Local hotspot learning", desc: "A campus mini-server distributes content to dozens of devices over LAN or Wi-Fi hotspot." },
  { icon: Smartphone, title: "Cross-device compatibility", desc: "Optimized for low-end Android phones, tablets, Chromebooks and shared lab computers." },
  { icon: Gauge, title: "Low-bandwidth optimized", desc: "Adaptive media, delta sync and aggressive caching keep data costs near zero." },
  { icon: Globe2, title: "TVET accessibility", desc: "Built-in Kiswahili UI, screen-reader labels, large text and high contrast modes." },
];
  ...
  return (
    <section className="container mx-auto px-4 py-20">
      <div className="mx-auto max-w-2xl text-center">
        ...
        <h2 className="text-3xl font-bold tracking-tight md:text-4xl">A learning platform that meets students where they are</h2>
        <p className="mt-3 text-muted-foreground">
          Every feature is designed for environments where internet is intermittent, devices are shared, and reliability matters more than novelty.
        </p>
      </div>
    ...
    <section className="bg-muted/40 py-20">
      <div className="container mx-auto px-4">
        <div className="mb-10 flex flex-wrap items-end justify-between gap-3">
          <div>
            <Badge variant="outline" className="mb-3">TVET tracks</Badge>
            <h2 className="text-3xl font-bold tracking-tight md:text-4xl">Competency-based learning across trades</h2>
            <p className="mt-2 max-w-xl text-muted-foreground">
              Practical workshops, lab sessions and assessments mapped to national TVET frameworks.
            </p>
          </div>
          <Button asChild variant="ghost">
            <Link to="/dashboard/courses">Browse all courses <ArrowRight className="ml-1 h-4 w-4" /></Link>
          </Button>
        </div>
        <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
          {categories.map((c) => (
            <Card key={c.name} className="group cursor-pointer border-border/60 bg-background transition hover:-translate-y-0.5 hover:border-accent/50 hover:shadow-soft">
...
const steps = [
  { n: "01", title: "Deploy", desc: "Install the JifunzeHub local server at your institution — a single small box per campus." },
  { n: "02", title: "Distribute", desc: "Students connect over LAN, Wi-Fi hotspot, or carry content home via USB packages." },
  { n: "03", title: "Sync", desc: "Whenever any device touches internet, JifunzeHub silently syncs progress and new content." },
];
  ...
  return (
    <section className="container mx-auto px-4 py-20">
      <div className="mx-auto max-w-2xl text-center">
        ...
        <h2 className="text-3xl font-bold tracking-tight md:text-4xl">Three layers of resilience</h2>
      </div>
...
const quotes = [
  { name: "Amina K.", role: "ICT Student, Kisumu Polytechnic", quote: "I finally completed my full networking module without ever buying mobile data. Everything just… works." },
  { name: "Mr. Otieno", role: "Lecturer, Electrical Engineering", quote: "I record once, package once, and 240 students across two campuses get the lesson the same week." },
  { name: "Dr. Wanjiru", role: "Principal, Nairobi TVET", quote: "JifunzeHub gave us a real digital strategy that actually fits our infrastructure reality." },
];
...
function Testimonials() {
  return (
    <section className="container mx-auto px-4 py-20">
      <div className="mx-auto max-w-2xl text-center">
        <Badge variant="outline" className="mb-3">Voices</Badge>
        <h2 className="text-3xl font-bold tracking-tight md:text-4xl">Educators and learners are seeing the difference</h2>
      </div>
      <div className="mt-12 grid gap-5 md:grid-cols-3">
        {quotes.map((q) => (
    ...
    <section className="container mx-auto px-4 pb-24">
      <div className="relative overflow-hidden rounded-3xl bg-gradient-hero p-10 text-primary-foreground shadow-elegant md:p-16">
        ...
        <div className="relative grid gap-6 md:grid-cols-[1fr_auto] md:items-center">
          <div>
            <h2 className="text-3xl font-bold tracking-tight md:text-4xl">Ready to bring your institution online — and offline?</h2>
            <p className="mt-3 max-w-2xl text-primary-foreground/85">
              Talk to our team about deploying JifunzeHub at your TVET college. Pilots typically run in under two weeks.
            </p>
          </div>
